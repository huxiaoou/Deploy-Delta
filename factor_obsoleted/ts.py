...
from solutions.math_tools import cal_roll_return, cal_ts, cal_ts_by_day
...
class CAlgFactorTS(CAlgFactor):
    def __init__(self, cfg: CCfgFactorTS):
    ...
    def update_factor_data_in_pre_trans_form(
        ...
        ts = cal_ts_by_day(ts_raw=ts_raw, win=self.cfg.args)
        self.add_to_factor(factor_data, "ts", code, ts)

    def cal_factor(self, *args, ts: pd.DataFrame, **kwargs) -> pd.Series:
        return cal_ts(ts)
